...
@section('title','Leave Request History By Admin')
@section('content')
...
<section class="content filter-row">
                        ...
                        <form action="{{ route('leave-request.admin-approve') }}" method="post">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-2">
                                    <!-- text input -->
                                    <div class="form-group" style="margin-top: 8px;">
                                        <label>Cancel Approve By Admin </label>
                                        <select class="form-control select2bs4" name="search_cancel_leave_approve_by_admi_manager" id="search_cancel_leave_approve_by_admi_manager" style="width: 100%;">
                                             <option value="">- Select -</option> 
                                            <option value="pending" {{ old('search_cancel_leave_approve_by_admi_manager') == 'pending' ? 'selected':''}}>- Pending -</option>
                                            <option value="reject" {{ old('search_cancel_leave_approve_by_admi_manager') == 'reject' ? 'selected':''}}>- Reject -</option>
                                            <option value="accept" {{ old('search_cancel_leave_approve_by_admi_manager') == 'accept' ? 'selected':''}}>- Accept -</option>
                                        </select>
                                    </div>
                                </div>
 ...
 <section class="content">
                        ...
                        <table id="leave_record" class="table table-hover">
                            ...
                            <tbody>
                               ...
                               ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                      <a class="text-primary" href="#" data-toggle="modal" data-target="#modal-detail"  data-cancel_leave_approve_reason_by_admi_manager="{{ $leave_request->cancel_leave_approve_reason_by_admi_manager }}"
                                        ...
                                        data-took_total="{{ $leave_request->took_total }}" data-day_type="{{ $leave_request->day_type }}" data-leave_day="{{ $leave_request->leave_day }}" data-approve_reason_by_gm="{{ $leave_request->approve_reason_by_GM }}" data-reason="{{ $leave_request->reason }}" data-remaining_days="{{ $leave_request->remaining_days }}"
                                            onclick="addValueForDetail(this)">
                                           {{ $leave_type_name }}
                                      </a>
                                    </td>
                                    @if(!empty($leave_request->certificate))
                                        <td><a class="btn btn-success btn-sm" href="{{ url('/leave-certificate/download/'.$leave_request->certificate ) }}">download file</a></td>
                                        @else
                                        <td></td>
                                        @endif
                                    <td>{{ $from_date }}</td>
                                ...
                                @foreach($leave_request_rs as $leave_request)
                                <?php
                                $fdate = str_replace('-"', '/', $leave_request->from_date);
                                  $from_date = date("d/m/Y", strtotime($fdate));
                               ...
                               ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                      <a class="text-primary" href="#" data-toggle="modal" data-target="#modal-rs-detail"  data-cancel_leave_approve_reason_by_admi_manager="{{ $leave_request->cancel_leave_approve_reason_by_admi_manager }}"
                                        data-id="{{ $leave_request->id }}"  data-department_name="{{ $leave_request->docname }}"  data-employee_name="{{ $leave_request->employee_name }}"  data-role_name="{{ $leave_request->role_name }}"  data-start_date="{{ $from_date}}" data-end_date="{{ $to_date}}" 
                                        data-leave_type_name="{{ $leave_type_name}}" data-photo="{{ url('/employee/'.$leave_request->photo_name ) }}" data-total="{{ $leave_request->total_days }}" data-approve_by_dep_manager="{{ $leave_request->approve_by_dep_manager }}"
                                       data-approve_by_rs_gm="{{ $leave_request->approve_by_RS_GM }}"  data-approve_by_gm="{{ $leave_request->approve_by_GM }}" data-updated_at="{{ $leave_request->updated_at }}"
                                       data-took_total="{{ $leave_request->took_total }}"  data-day_type="{{ $leave_request->day_type }}" data-leave_day="{{ $leave_request->rs_earned_leaves }}"  data-approve_reason_by_gm="{{ $leave_request->approve_reason_by_GM }}" data-reason="{{ $leave_request->reason }}" data-remaining_days="{{ $leave_request->remaining_days }}"
                                            onclick="addValueForRSDetail(this)">
                                           {{ $leave_type_name }}
                                      </a>
                                    </td>
                                    @if(!empty($leave_request->certificate))
                                    ...
                                    <td></td>
                                    <td>{{ $leave_request->approve_by_GM }}</td>
                                    <td>{{ $leave_request->approve_by_RS_GM }}</td>
                                    <td>{{ $leave_request->remaining_days }}</td>
                                    <td>{{ $leave_request->cancel_leave_approve_by_dep_manager }}</td>
                                    <td><a href="#"  data-id="{{ $leave_request->id }}" data-toggle="modal" data-target="#modal-cancel"
                                    data-leave_cancel_reason="{{ $leave_request->leave_cancel_reason }}"  data-cancel_leave_approve_reason_by_admi_manager="{{ $leave_request->cancel_leave_approve_reason_by_admi_manager }}" onclick="addValueForCancel(this)">{{ $leave_request->cancel_leave_approve_by_admi_manager }}</a></td>
    ...
    <!-- start for rs -->
      <div class="modal fade" id="modal-rs-detail">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Apply Leave Form</h4>
                ...
                <form action="../../pages/mastermanagement/allowance-list.html" method="post">
                    <div class="modal-body">
                        <input type="hidden" name="rs_leave_id" class="rs_leave_request_id">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 text-right">
                                <img style="width:100px;height:100px" class="img-circle profile_photo" />
                            </div>
                            <div class="col-md-6 col-sm-6 text-left">
                                <span class="rs_employee_name"></span><br>
                                <span class="rs_role_name"></span><br>
                                <span class="rs_department_name"></span><br>
                            </div>
                        </div>
                        <div class="row p-2">
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Start Date :</span>
                                <span class="text-primary rs_start_date"></span>
                            </div>
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">End Date :</span>
                                <span class="text-primary rs_end_date"></span>
                            </div>
                        </div>
                        <div class="row p-2">
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Request Days :</span>
                                <span class="text-primary rs_total"></span>
                            </div>
                            <div class="col-md-6 col-sm-6">
                                <!-- <span class="text-bold">Remaining Days :</span>
                                <span class="text-primary before_remaining_days"></span> -->
                                 ...
                                 <span class="text-bold"> Taken Leave Days :</span>                                    
                                <span class="text-primary rs_second_total"></span>
                                <span class="text-primary"> / </span>
                                <span class="text-primary rs_before_remaining_days"></span>
                        ...
                        <div class="row p-2">
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Remaining Days :</span>
                                <span class="text-primary rs_after_remaining_days"> </span>
                            </div>
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Leave Type :</span>
                                <span class="text-primary rs_leave_type_name"></span>
                            </div>
                        </div>
                        <div class="row p-2">
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Approve By ADMI :</span>
                                <span class="text-primary rs_approve_by_gm"></span>
                            </div>
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Approve By GM :</span>
                                <span class="text-primary rs_approve_by_rs_gm"></span>
                            </div>
                        </div>
                        <div class="row p-2">
                            <div class="col-md-6 col-sm-6">
                                    <span class="text-bold">Day Type :</span>
                                    <span class="text-primary rs_day_type"></span>
                                </div>
                                
                            <div class="col-md-6 col-sm-6">
                                <span class="text-bold">Submitted At :</span>
                                <span class="text-primary rs_updated_at"> </span>
                            </div>
                        </div>
                        <div class="row p-2">
                            <div class="col-md-12 col-sm-12">
                        ...
                        <div class="row p-2">
                             <label for="reason" class="col-form-label">Enter Accept / Reject Reason</label>
                             <textarea class="form-control" name="rs_approve_by_rs_gm" id="rs_approve_by_rs_gm"  placeholder="enter reason for reject"></textarea>
                         </div>
                         
                    </div>
                    <div class="modal-footer justify-content-between">
                        <button type="button" class="btn btn-success rs-accept-btn">Accept</button>
                        <button type="button" class="btn btn-danger rs-reject-btn">Reject</button>
    ...
    <script type="text/javascript">
       ...
       //leave-cancel
        ...
        success:function(data){
           ...
           //accept
        $('.rs-accept-btn').click(function(){
        var id=$('.rs_leave_request_id').val();
        var approve_reason_by_GM = $("#rs_approve_reason_by_GM").val();
        $.ajax
        ({
        type: "GET",
        url: "{{ route('leave-request.rs-admin-change-status') }}",
        data: {"status":'accept',"id": id,"approve_reason_by_GM":approve_reason_by_GM},
        success:function(data){
            if($.isEmptyObject(data.error)){
                    alert(data.success);
  ...
  $("#leave_type_id").change(function() {
   ...
   //start for rs   
    function addValueForRSDetail(btn){

       var from_date=$(btn).data('start_date');
       var to_date=$(btn).data('end_date');
       var leave_type_name=$(btn).data('leave_type_name');
       var total=$(btn).data('total');
       var approve_by_rs_gm=$(btn).data('approve_by_rs_gm');
       var approve_by_gm=$(btn).data('approve_by_gm');
       var updated_at=$(btn).data('updated_at');
       var reason=$(btn).data('reason');
       var remaining_days=$(btn).data('remaining_days');
     ...
     //  var after_remaining_days = $(btn).data('remaining_days') -  $(btn).data('total');
       ...
       $(".rs_leave_request_id").val(id);
       $(".rs_start_date").text(from_date);
       $(".rs_end_date").text(to_date);
       $(".rs_leave_type_name").text(leave_type_name);
       $(".rs_total").text(total);
       $(".rs_approve_by_rs_gm").text(approve_by_rs_gm);
       $(".rs_approve_by_gm").text(approve_by_gm);
       $(".rs_updated_at").text(updated_at);
       $(".rs_reason").text(reason);
    //    $(".before_remaining_days").text(remaining_days);
    //    $(".after_remaining_days").text(after_remaining_days);
        $(".rs_before_remaining_days").text(leave_day);
        $(".rs_second_total").text(second_total);
        $(".rs_after_remaining_days").text(after_remaining_days);
       $(".rs_role_name").text(role_name);
       $(".rs_employee_name").text(employee_name);
       $(".rs_department_name").text(department_name);
       $(".rs_profile_photo").attr("src",photo_name);
       $("#rs_approve_reason_by_GM").val(approve_reason_by_gm);
       $(".rs_day_type").text(day_type);
