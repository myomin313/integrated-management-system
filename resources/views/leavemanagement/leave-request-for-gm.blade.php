...
@section('title','Leave Request History By GM')
@section('content')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <form action="{{ route('leave-requests.approve-by-gm') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-sm-2">
                                    <!-- text input -->
                                ...
                                <div class="col-sm-2">
                                    <!-- text input -->
                                    <div class="form-group" style="margin-top: 8px;">
                                        <label>Cancel Approve By GM </label>
                                        <select class="form-control select2bs4" name="search_cancel_leave_approve_by_RS_GM" id="search_cancel_leave_approve_by_RS_GM" style="width: 100%;">
                                             <option value="">- Select -</option> 
                                            <option value="pending" {{ old('search_cancel_leave_approve_by_RS_GM') == 'pending' ? 'selected':''}}>- Pending -</option>
                                            <option value="reject" {{ old('search_cancel_leave_approve_by_RS_GM') == 'reject' ? 'selected':''}}>- Reject -</option>
                                            <option value="accept" {{ old('search_cancel_leave_approve_by_RS_GM') == 'accept' ? 'selected':''}}>- Accept -</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>Leave Approve By GM</th>
                                    <th>Remaining Days</th>
                                    <th>Leave Cancel Approve By GM</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                      <a class="text-primary" href="#" data-toggle="modal" data-target="#modal-detail"  data-cancel_leave_approve_reason_by_admi_manager="{{ $leave_request->cancel_leave_approve_reason_by_admi_manager }}"
                                        data-id="{{ $leave_request->id }}"  data-department_name="{{ $leave_request->docname }}"  data-employee_name="{{ $leave_request->employee_name }}"  data-role_name="{{ $leave_request->role_name }}"  
                                        data-start_date="{{ $from_date}}" data-end_date="{{ $to_date}}" 
                                        data-leave_type_name="{{ $leave_type_name}}" data-photo="{{ url('/employee/'.$leave_request->photo_name ) }}" data-total="{{ $leave_request->total_days }}" 
                                        data-leave_day="{{ $leave_request->leave_day }}" data-approve_by_RS_GM="{{ $leave_request->approve_by_RS_GM }}"
                                        data-day_type="{{ $leave_request->day_type }}" data-approve_by_gm="{{ $leave_request->approve_by_GM }}" data-updated_at="{{ $leave_request->updated_at }}"
                                        data-took_total="{{ $leave_request->took_total }}"  data-approve_reason_by_rs_gm="{{ $leave_request->approve_reason_by_RS_GM }}" data-reason="{{ $leave_request->reason }}" data-remaining_days="{{ $leave_request->remaining_days }}"
                                    ...
                                    <!--<td>{{ $leave_request->cancel_leave_approve_by_RS_GM  }}</td>-->
                                    
                                     <td><a href="#"  data-id="{{ $leave_request->id }}" data-toggle="modal" data-target="#modal-cancel"
                                    data-leave_cancel_reason="{{ $leave_request->leave_cancel_reason }}" 
                                     data-cancel_leave_approve_reason_by_RS_GM="{{ $leave_request->cancel_leave_approve_reason_by_RS_GM }}" onclick="addValueForCancel(this)">{{ $leave_request->cancel_leave_approve_by_RS_GM }}</a></td>
                        ...
                        <div class="row">
                         ...
                         <div class="row">
                            <div class="col-md-12 col-sm-12">
                                ...
                                <textarea name="cancel_leave_approve_reason_by_RS_GM" id="cancel_leave_approve_reason_by_RS_GM"
                                class="form-control">
                              </textarea>
                            </div>
                        </div>
  ...
  <!-- /.container-fluid -->
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row p-2">
                            ...
                            <div class="col-md-6 col-sm-6">
                                ...
                                <span class="text-primary approve_by_RS_GM"></span>
                            </div>
                        </div>
                        <div class="row p-2">
                             <div class="col-md-6 col-sm-6">
                        ...
                        <div class="row p-2">
                             ...
                             <textarea class="form-control" name="approve_reason_by_RS_GM" id="approve_reason_by_RS_GM"  placeholder="enter reason for reject"></textarea>
                         </div>
    ...
    <script type="text/javascript">
      ...
      $('.file_danger').hide();
        ...
        var cancel_leave_approve_reason_by_RS_GM = $('#cancel_leave_approve_reason_by_RS_GM').val();
        $.ajax
        ({
        type: "GET",
        url: "{{ route('leave-request.leave-cancel-gm-change-status') }}",
        data: {"status":'accept',"id": id,"cancel_leave_approve_reason_by_RS_GM":cancel_leave_approve_reason_by_RS_GM},
       ...
       //leave-cancel
        ...
        type: "GET",
        url: "{{ route('leave-request.leave-cancel-gm-change-status') }}",
        data: {"status":'reject',"id": id,"cancel_leave_approve_reason_by_RS_GM":cancel_leave_approve_reason_by_RS_GM},
        success:function(data){
            if($.isEmptyObject(data.error)){
                    alert(data.success);
                     location.reload();
        ...
        var approve_reason_by_RS_GM = $("#approve_reason_by_RS_GM").val();
        $.ajax
        ({
        type: "GET",
        url: "{{ route('leave-request.gm-change-status') }}",
        data: {"status":'accept',"id": id,"approve_reason_by_RS_GM":approve_reason_by_RS_GM},
        success:function(data){
            if($.isEmptyObject(data.error)){
                    alert(data.success);
                     location.reload();
       ...
       //reject
        ...
        type: "GET",
        url: "{{ route('leave-request.gm-change-status') }}",
        data: {"status": 'reject',"id": id,"approve_reason_by_RS_GM":approve_reason_by_RS_GM},
        success:function(data){
            if($.isEmptyObject(data.error)){
                    alert(data.success);
                     location.reload();
...
//create script start
  ...
  $("#leave_type_id").change(function() {
   ...
   function addValueForDetail(btn){
       ...
       var approve_reason_by_rs_gm=$(btn).data('approve_reason_by_rs_gm');
    ...
    //  var gday = leave_day -  remaining_days;
     ...
     // var second_total = gday + total; 
       ...
       $(".total").text(total);
       $(".approve_by_RS_GM").text(approve_by_rs_gm);
    ...
    //    $(".after_remaining_days").text(after_remaining_days);
       ...
       $("#approve_reason_by_RS_GM").val(approve_reason_by_rs_gm);
