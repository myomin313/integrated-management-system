...
class RSActualTaxPaymentExport implements FromCollection,WithHeadings,WithStyles,ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        foreach($record as $key=>$value){
            ...
            $data[]=[
	                ...
	                getUserFieldWithId($value->user_id,"passport_number"),
	                '',
	                getPositionName(getUserFieldWithId($value->user_id,"position_id")),
	                siteformat_number($value->tax_amount_usd),
	                siteformat_number($value->exchange_rate),
	                siteformat_number($value->tax_amount_mmk),
	                Carbon::parse($value->date)->format("F, Y").' RS'
