	    ...
	    $changestatuses= DB::transaction(function () use ($input,$status,$ip) {
			...
			if($status==1){
				...
				if($attendance){
					...
					if($change->working_start_time)
						$attendance->corrected_start_time = $change->working_start_time;
					if($change->working_end_time)
						$attendance->corrected_end_time = $change->working_end_time;
					...
					if($working_start_time)
						$attendance->corrected_start_time = $change->working_start_time?$change->working_start_time:$working_start_time;
					else{
						if($change->working_start_time)
							$attendance->corrected_start_time = $change->working_start_time;
						else{
					...
					if($working_end_time)
						$attendance->corrected_end_time = $change->working_end_time?$change->working_end_time:$working_end_time;
					else{
						if($change->working_end_time)
							$attendance->corrected_end_time = $change->working_end_time;
						else{
