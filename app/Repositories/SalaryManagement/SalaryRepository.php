...
class SalaryRepository extends BaseRepository
{
	...
	public function index($name,$order){
		if(Auth::user()->can("salary-read-all"))
			return $this->model->select("salaries.*")->orderBy($name,$order);
			...
			return $this->model->select("salaries.*")->whereIn('salaries.user_id',$user_ids)->orderBy($name,$order);
		}
		else
			return $this->model->select("salaries.*")->where('salaries.user_id','=',Auth::user()->id)->orderBy($name,$order);
	...
	public function storeNS($input){
		$salaries= DB::transaction(function () use ($input) {
			...
			$salary->previous_normal_ot_hr = $input['previous_normal_ot_hr'];
			$salary->previous_sat_ot_hr = $input['previous_sat_ot_hr'];
			$salary->previous_sunday_ot_hr = $input['previous_sun_ot_hr'];
			$salary->previous_public_ot_hr = $input['previous_public_holiday_ot_hr'];

			if(isset($input['is_retire'])){
				$salary->current_normal_ot_hr = isset($input['current_normal_ot_hr'])?$input['current_normal_ot_hr']:"00:00";
				$salary->current_sat_ot_hr = isset($input['current_sat_ot_hr'])?$input['current_sat_ot_hr']:"00:00";
				$salary->current_sunday_ot_hr = isset($input['current_sun_ot_hr'])?$input['current_sun_ot_hr']:"00:00";
				$salary->current_public_ot_hr = isset($input['current_public_holiday_ot_hr'])?$input['current_public_holiday_ot_hr']:"00:00";
			}
			else{
			...
			$salary->total_ot_hr = $input['total_ot_hr'];
			...
			$salary->ot_rate_usd = $input["ot_rate_usd"];

			$salary->previous_normal_ot_payment_usd = $input['previous_normal_ot_payment_usd'];
			$salary->previous_sat_ot_payment_usd = $input['previous_sat_ot_payment_usd'];
			$salary->previous_sunday_ot_payment_usd = $input['previous_sunday_ot_payment_usd'];
			$salary->previous_public_ot_payment_usd = $input['previous_public_ot_payment_usd'];
			$salary->previous_taxi_charge_usd = $input['previous_taxi_charge_usd'];
			if(isset($input['is_retire'])){
				$salary->current_normal_ot_payment_usd = isset($input['current_normal_ot_payment_usd'])?$input['current_normal_ot_payment_usd']:0;
				$salary->current_sat_ot_payment_usd = isset($input['current_sat_ot_payment_usd'])?$input['current_sat_ot_payment_usd']:0;
				$salary->current_sunday_ot_payment_usd = isset($input['current_sunday_ot_payment_usd'])?$input['current_sunday_ot_payment_usd']:0;
				$salary->current_public_ot_payment_usd = isset($input['current_public_ot_payment_usd'])?$input['current_public_ot_payment_usd']:0;
				$salary->current_taxi_charge_usd = isset($input['current_taxi_charge_usd'])?$input['current_taxi_charge_usd']:0;
			}
			else{
			...
			$salary->total_ot_payment_usd = $input["ot_usd"];
			$salary->total_ot_payment_mmk = $input["ot_mmk"];

			$salary->unpaid_leave_day = $input["unpaid_leave_day"];
	...
	public function paySlipList($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		$query = $this->index("salaries.salary_usd","desc");
		
		if($from_date!=null){
			$from_date = "01/".$from_date;
            $query->whereRaw( " CAST(salaries.pay_date AS DATE) >= STR_TO_DATE(\"$from_date\", '%d/%m/%Y')");
        }
        if($to_date!=null){
        	$to_date = "31/".$to_date;
            $query->whereRaw( " CAST(salaries.pay_date AS DATE) <= STR_TO_DATE(\"$to_date\", '%d/%m/%Y')");
        }
        if($employee!=null){
			$query->where(function($q) use ($employee){
				$q->where('salaries.user_id','=',$employee);
			});
		}
	...
	public function indexSalary($name,$order){
		if(Auth::user()->can("salary-read-all"))
			return $this->model->select("*")->orderBy('users.employee_type_id',"asc")->orderBy($name,$order);
		else if(Auth::user()->can("salary-read-group")){
			//$user_ids = User::where('department_id','=',Auth::user()->department_id)->pluck('id');
			$terms = explode(',',auth()->user()->department_id);
		    $user_ids = array();
			...
			return $this->model->select("salaries.*")->whereIn('salaries.user_id',$user_ids)->orderBy('users.employee_type_id',"asc")->orderBy($name,$order);
		}
		else
			return $this->model->select("salaries.*")->where('salaries.user_id','=',Auth::user()->id)->orderBy('users.employee_type_id',"asc")->orderBy($name,$order);
		
	}
	public function monthlySalaryList($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		$query = $this->indexSalary("salaries.salary_usd","desc");
		$query->where("employee_type","!=","rs");
		$query->leftJoin("users","users.id","=","salaries.user_id");
		if($from_date!=null){
			$from_date = "01/".$from_date;
            $query->whereRaw( " CAST(salaries.pay_date AS DATE) >= STR_TO_DATE(\"$from_date\", '%d/%m/%Y')");
        }
	...
	public function paySlipBank($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		$query = $this->index("salaries.salary_usd","desc");
		$query->where("employee_type","!=","rs");
		if($from_date!=null){
			$from_date = "01/".$from_date;
	...
	public function paySlipNS($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		$query = $this->index("salaries.salary_usd","desc");
		$query->where("employee_type","!=","rs");
		if($from_date!=null){
			$from_date = "01/".$from_date;
	...
	public function paySlipRS($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		$query = $this->index("salaries.salary_usd","desc");
		$query->where("employee_type","=","rs");
		if($from_date!=null){
			$from_date = "01/".$from_date;
