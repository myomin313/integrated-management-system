...
class RsTaxRepository extends BaseRepository
{
	protected $model_log;
	public function __construct(RsActualTax $tax)
	{
		$this->model=$tax;
	}
	...
	public function index($name,$order){
		...
		return $this->model->select('rs_actual_taxes.*')->orderBy($name,$order);
	...
	public function indexOrder($name,$order,$from_date=null,$to_date=null,$employee=null,$n){
		...
		$query = $this->model->select('rs_actual_taxes.*',DB::raw("(select $month from rs_salaries where rs_actual_taxes.user_id = rs_salaries.user_id and salary_type='mm_salary' and year=$year) AS salary"))->orderBy("salary","desc");
		...
		if(Auth::user()->can("tax-read-group")){
		    ...
		    $query->whereIn("rs_actual_taxes.user_id",$user_ids);
		}
		else if(Auth::user()->can("tax-read-one")){
			$query->where("rs_actual_taxes.user_id","=",Auth::user()->id);
		}

		if($from_date!=null){
            $query->whereRaw( " CAST(rs_actual_taxes.pay_date AS DATE) >= STR_TO_DATE(\"$from_date\", '%d/%m/%Y')");
        }
        if($to_date!=null){
            $query->whereRaw( " CAST(rs_actual_taxes.pay_date AS DATE) <= STR_TO_DATE(\"$to_date\", '%d/%m/%Y')");
        }
        if($employee!=null){
	...
	public function store($input){
		$taxes = DB::transaction(function () use ($input){
			...
			foreach($user_id as $key=>$id){
				$tax = RsActualTax::where([["user_id","=",$id],["tax_for","=",$tax_for[$key]]])->first();
				if(!$tax)
					$tax = new RsActualTax();
				else{
				...
				$rs_income_tax = RsIncomeTax::where([["user_id","=",$tax->user_id],["date","=",$end_date]])->first();
				if($rs_income_tax){
					$rs_income_tax->actual_tax_mmk = $tax->tax_amount_mmk;
					$rs_income_tax->actual_exchange_rate = $tax->exchange_rate;
					$rs_income_tax->actual_tax_usd = $tax->tax_amount_usd;
					$rs_income_tax->save();
				}
			}
